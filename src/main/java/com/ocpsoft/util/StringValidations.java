...
public class StringValidations
{
   ...
   public static boolean isState(final String state)
   {
      if (state != null)
      {
         return state.toUpperCase().matches("^(AL|AK|AS|AZ|AR|CA|CO|CT|DE|DC|FM|FL|FA|GU|HI|ID|IL|IN|IA|KS|KY|LA|ME|MH|MD|" + "MA|MI|MN|MS|MO|MT|NE|NV|NH|NJ|NM|NY|NC|ND|MP|OH|OK|OR|PW|PA|PR|RI|SC|SD|TN|TX|" + "UT|VT|VI|VA|WA|WV|WI|WY)$");
      }
   ...
   public static boolean isPassword(final String password)
   {
      if (password != null)
      {
         return password.matches("^[a-zA-Z0-9!@#$%^&*\\s\\(\\)_\\+=-]{4,}$");
      }
